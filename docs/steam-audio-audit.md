...
- [Valve/Steam, 2026-06-14, Steam appdetails API for AppID 3678970: `https://store.steampowered.com/api/appdetails?appids=3678970&filters=basic`]
- [Valve/Steam CDN, 2026-06-14, Trailer HLS manifest extracted from the Store page]
...
- Verifies that each referenced WAV exists and that there are no extra unreferenced WAV files in `Sources/Resources/Extracted/sfx/`.
...
- Uses an injected recording audio client to verify Settings preview, manual equip, chest opening, Synthesis, offline level-up, sound mute, and sound re-enable behavior without playing real sound during the test run.
...
- Packaged WAV files under `Sources/Resources/Extracted/sfx/` are original generated substitutes, not official extracted audio.
- `GameAudio` prefers packaged SFX and falls back to macOS system sounds only when a resource is missing.
- `ResourceSelfTest` validates event coverage, rejects unreferenced bundled WAV files, checks representative `BattleEvent` routes through `GameEngine.audioEvent(for:)`, checks playability, 16-bit PCM encoding, mono channel count, 22050 Hz sample rate, 0.05-0.75s SFX duration, RMS level and peak level, verifies every `GameAudioEvent.volume` against local runtime-volume categories, and verifies every `GameAudioEvent.minimumInterval` against the same local throttle categories used by the shell audit and DEBUG self-test.
- `scripts/audit-local-sfx.sh` quantifies packaged event SFX against the same broad technical boundaries, statically audits runtime routing, and now applies event-profile checks for duration, envelope activity, onset, zero-crossing rate, coarse spectral centroid, high/low-frequency energy ratio, duplicate payloads, cross-event relationships, per-event runtime playback volume, runtime-volume sequence mixes, and runtime playback throttles. Current local results cover all 10 `GameAudioEvent` sounds and all 7 `BattleEvent` routes, with 16-bit PCM mono 22050 Hz WAV files, duration range `0.110-0.360s`, RMS range `-19.5..-11.2 dBFS`, peak range `-11.1..-6.5 dBFS`, coarse spectral-centroid range `553..3173 Hz`, runtime-volume range `0.36-0.46`, and minimum-interval range `0.18-0.50s`. The current sequence mixes measure `combat_burst` at `0.790s`, `-21.5 dBFS` RMS and `-14.0 dBFS` peak; `reward_chain` at `1.440s`, `-23.2 dBFS` RMS and `-16.0 dBFS` peak; and `defeat_chain` at `0.530s`, `-22.6 dBFS` RMS and `-14.0 dBFS` peak. Current runtime volumes are `0.42` for repeatable combat events, `0.36` for loot/equip/preview UI events, and `0.46` for victory/defeat/level-up cues. Current runtime throttles are `0.18s` for repeatable combat events, `0.25s` for loot/equip/preview UI events, and `0.50s` for victory/defeat/level-up cues.
