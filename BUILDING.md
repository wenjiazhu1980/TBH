...
build → swift test（完整套件）→ self-test → resource-self-test → 本地玩法/装备图标/被动技能图标/英雄精灵/音效复核脚本 → 打包 TBH.app 并上传 artifact。

`nightly.yml` 还会运行 `RENDER_BATTLE_SCENE=1 scripts/audit-local-battle-scene.sh`，对 deterministic SwiftUI 战斗场景渲染做像素级回归检查。
...
- `scripts/audit-local-*.sh`：本地/CI 复核脚本，覆盖玩法来源覆盖率、装备图标清洁度、被动技能源图标、音效技术指标、英雄精灵和战斗场景渲染。`audit-local-item-icons.sh`、`audit-local-passive-skill-icons.sh` 与 `audit-local-hero-sprites.sh` 需要 Python Pillow。
